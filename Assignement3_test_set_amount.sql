...
# People who donated in 2017
insert into Ass3_contacts_test
select distinct contact_id
  from acts
  where year(act_date) = 2017;
 
 select count(*) from Ass3_contacts_test;  # 98320
 
######################### previous code ###############################

drop table Ass3_test_my_dataset;

CREATE TABLE Ass3_test_my_dataset (
  `contact_id` int(10) unsigned ,
  `recency` float,
...
insert into ass3_test_my_dataset_PA
 ...
 join 
  ( SELECT a.contact_id  as 'contact_id',
         ...
         LEFT JOIN (SELECT contact_id, COUNT(amount) AS counter
                    ...
                    GROUP BY contact_id) AS c
         ON c.contact_id = a.contact_id
         WHERE (act_type_id = 'PA') AND (act_date < 20170101)
         GROUP BY 1
  ) b
on b.contact_id = assignment2.contact_id;

select count(*) from ass3_test_my_dataset_PA;

-- My_dataset_PA_DO
drop table Ass3_test_my_dataset_PA_DO;
CREATE TABLE Ass3_test_my_dataset_PA_DO (
  `contact_id` int(10) unsigned ,
  `DO_recency` float,
...
insert into Ass3_test_my_dataset_PA_DO
select my_dataset.contact_id as contact_id,
 ...
 from ass3_test_my_dataset       my_dataset
 left
 join ass3_test_my_dataset_PA    my_dataset_PA
...
drop table Ass3_test_Contact_Sol_acc;
CREATE TABLE Ass3_test_Contact_Sol_acc (
  `contact_id` int(10) unsigned ,
 ...
 -- succesful messages
 insert into Ass3_test_Contact_Sol_acc
 select z.contact_id,
...
drop table ass3_test_my_dataset_PA_DO_Sol;
-- Add all the features and NULL values
CREATE TABLE ass3_test_my_dataset_PA_DO_Sol (
  `contact_id` int(10) unsigned ,
  `DO_recency` float,
  `DO_frequency` integer(5) DEFAULT NULL,
  `DO_avgamount` float DEFAULT NULL,
...
insert into ass3_test_my_dataset_PA_DO_Sol
select my_dataset_PA_DO.contact_id, 
  ...
  from ass3_test_my_dataset_PA_DO  my_dataset_PA_DO
  left 
  join ass3_test_Contact_Sol_acc  Contact_Sol_acc
...
drop table ass3_test_my_dataset_PA_DO_Sol_label;

CREATE TABLE ass3_test_my_dataset_PA_DO_Sol_label (
  `contact_id` int(10) unsigned ,
  `DO_recency` float,
  `DO_frequency` integer(5) DEFAULT NULL,
  `DO_avgamount` float DEFAULT NULL,
...
insert into ass3_test_my_dataset_PA_DO_Sol_label
select 
 ...
 from ass3_test_my_dataset_PA_DO_Sol     my_dataset_PA_DO_Sol
inner 
 join Ass3_contacts_test              assignment2
   on assignment2.contact_id = my_dataset_PA_DO_Sol.contact_id;

select count(*) from ass3_test_my_dataset_PA_DO_Sol_label;  # 236885
select * from ass3_test_my_dataset_PA_DO_Sol_label;
--------------------- Round 2 -------------------------
...
drop table ass3_test_sol_accuracy_measures;
CREATE TABLE ass3_test_sol_accuracy_measures (   -- exei noima na spasei se DO kai PA?
  `contact_id` int(10) unsigned ,
...
insert into ass3_test_sol_accuracy_measures
select assignment2.contact_id as contact_id,
...
drop table ass3_test_Solicitation_Efficiency;
 CREATE TABLE  ass3_test_Solicitation_Efficiency(       -- ana epitiximena campaigns posa solicitations dextike
  `contact_id` int(10) unsigned ,
...
insert into ass3_test_Solicitation_Efficiency
select assignment2.contact_id  as contact_id,
...
select contact_id, campaign_id
  ...
  and year(actions.action_date) <= 2016
...
drop table ass3_test_info_DO_2018;
# It actually has data from 2017
CREATE TABLE ass3_test_info_DO_2018 (
  `contact_id`           int(10) unsigned ,
...
insert into ass3_test_info_DO_2018
select assignment2.contact_id   as contact_id,
...
drop table ass3_test_info_PA_2018;
CREATE TABLE ass3_test_info_PA_2018 (
  `contact_id` int(10) unsigned ,
...
insert into ass3_test_info_PA_2018
select assignment2.contact_id,
 ...
 # select * from   ass3_test_info_PA_2018;

drop table ass3_test_DO_Average_amounts;
CREATE TABLE ass3_test_DO_Average_amounts (
  `contact_id` int(10) unsigned ,
...
insert into ass3_test_DO_Average_amounts
select assignment2.contact_id,
	   x.avg_min_amount,
       x.avg_max_amount,
       x.avg_avg_amount
...
select contact_id, year(act_date), 
  ...
  from acts
  where acts.act_type_id = 'DO' and year(act_date) <= 2016
...
drop table ass3_test_DO_Sum_amount_across_years;
 CREATE TABLE ass3_test_DO_Sum_amount_across_years (
  `contact_id` int(10) unsigned ,
...
insert into ass3_test_DO_Sum_amount_across_years
 select assignment2.contact_id , 
...
drop table ass3_test_DO_Frequency_across_years;
   CREATE TABLE ass3_test_DO_Frequency_across_years (
  `contact_id` int(10) unsigned ,
...
insert into ass3_test_DO_Frequency_across_years
  select assignment2.contact_id,
         min_freq_years_don,
         max_freq_years_don,
		 avg_freq_years_don
...
drop table ass3_test_my_dataset_latest;
CREATE TABLE ass3_test_my_dataset_latest (
  `contact_id` int(10) unsigned ,
  `DO_recency` float,
  `DO_frequency` integer(5) DEFAULT NULL,
  `DO_avgamount` float DEFAULT NULL,
...
insert into ass3_test_my_dataset_latest
select my_dataset_PA_DO_Sol_label.contact_id                                   ,
  ...
  from ass3_test_my_dataset_pa_do_sol_label  my_dataset_pa_do_sol_label
  left
  join ass3_test_sol_accuracy_measures  sol_accuracy_measures
    on sol_accuracy_measures.contact_id = my_dataset_pa_do_sol_label.contact_id
  left
  join ass3_test_Solicitation_Efficiency  Solicitation_Efficiency
    on Solicitation_Efficiency.contact_id = my_dataset_pa_do_sol_label.contact_id
  left
  join ass3_test_info_DO_2018   info_DO_2018
    on info_DO_2018.contact_id = my_dataset_pa_do_sol_label.contact_id
  left
  join ass3_test_info_PA_2018   info_PA_2018
    on info_PA_2018.contact_id = my_dataset_pa_do_sol_label.contact_id
 left
 join ass3_test_DO_Average_amounts   DO_Average_amounts
   on DO_Average_amounts.contact_id = my_dataset_pa_do_sol_label.contact_id
 left
 join ass3_test_DO_Sum_amount_across_years  DO_Sum_amount_across_years
   on DO_Sum_amount_across_years.contact_id = my_dataset_pa_do_sol_label.contact_id
 left
 join ass3_test_DO_Frequency_across_years  DO_Frequency_across_years
   on DO_Frequency_across_years.contact_id = my_dataset_pa_do_sol_label.contact_id;
   
   
 --------------- Round 3 ----------------- 
 drop table ass3_test_Customer_Active;
 CREATE TABLE ass3_test_Customer_Active (
  `contact_id` int(10) unsigned ,
...
-- bring active
insert into ass3_test_Customer_Active
...
-- prefixes
  drop table ass3_test_Prefix_table;
	CREATE TABLE ass3_test_Prefix_table (
  `contact_id` int(10) unsigned ,
  `Autre` boolean,
  `Docteur` boolean,
  `Maitre` boolean,
...
insert into ass3_test_Prefix_table
select assignment2.contact_id as contact_id,
...
select sum(Autre), sum(Docteur), sum(Maitre), sum(Mademoiselle), sum(Madame), sum(Monsieur_et_Madame), sum(Monsieur), sum(NA)
  from Ass3_contacts_test   assignment2 
  left
  join ass3_test_Prefix_table  prefix_table
    on assignment2.contact_id = prefix_table.contact_id; 
  

-- Customers channel usage

drop table ass3_test_Customer_channel_cat;
CREATE TABLE ass3_test_Customer_channel_cat (
  `contact_id`        int(10) unsigned ,
  `Mailing`           integer,
  `Internet`          integer,
  `Street`            integer,
...
insert into ass3_test_Customer_channel_cat
select a.contact_id,
...
select assignment2.contact_id,
  ...
  join acts acts
    on acts.contact_id = assignment2.contact_id
    where year(acts.act_date) <= 2017 ) as a
  group by contact_id;

# drop table ass3_test_my_dataset_latest2;
drop table ass3_test_my_dataset_latest2;
CREATE TABLE ass3_test_my_dataset_latest2 (
  `contact_id` int(10) unsigned ,
  `DO_recency` float,
  `DO_frequency` integer(5) DEFAULT NULL,
  `DO_avgamount` float DEFAULT NULL,
...
insert into ass3_test_my_dataset_latest2
select 
...
Customer_channel_cat.Autre       
  from ass3_test_my_dataset_latest  my_dataset_latest
  left
  join ass3_test_Customer_Active     Customer_Active
    on Customer_Active.contact_id = my_dataset_latest.contact_id
  left
  join ass3_test_Prefix_table       Prefix_table
    on Prefix_table.contact_id = my_dataset_latest.contact_id
  left
  join ass3_test_Customer_channel_cat   Customer_channel_cat
...
select ass3_test_my_dataset_latest2.contact_id, avg(acts.amount) as avg_amount
 ...
 #####################################    END      ###################################################    
select * from ass3_test_my_dataset_latest2;

------ WHERE YEAR >= 1991

-- Additional Feratures  (ta sapw ola se 'DO' kai 'PA')  !!!!
                          -- kovw pistr 
		-- frequency of solicitations for C189 (actions)
		-- solicitation accuracy (distinct payments/ distinct campaigns) - sol_acc2, avg_accuracy per succesful solicitation (0,2 + 1)/2 = 0,6 (an thewrisoume oti sou stelnoun solicitation mexri na kaneis donation)
		-- frequency (min, max,median,avg) of transactions per year - across the years (frequency(count) kai amount (sum)), (transactions already done )  (fores i amount)  -- xronia 206, 2017 (across all years)
		-- gia to 2018 ti exei kanei mexri stigmis 
		-- stiles oles oi xronies kai transactions ana xronia  
		-- outliers pisw apo 1990	
		-- channel_id, payment_method_id count ana pelati ana meso (pivot)	  !!!!!
		
		
		-- thelw na kanw to idio gia to test-set
		
		-- zip code- city
		-- prefix   !!!!
        
	select count(*)
      from my_dataset_latest2
      where calibration = 0;
      
	select count(*)
      from my_dataset_latest2
      where donation_flg is null;
